...
function getCapacityFromSelect(roomSelect) {
  const selected = roomSelect.options[roomSelect.selectedIndex];
  return selected && selected.dataset.capacity ? Number(selected.dataset.capacity) : 0;
}

function updateBookingOverview(capacity, occupiedSeats) {
  ...
  if (!capacity) {
    ...
    const available = capacity - occupiedSeats.length;
    seatsAvailableEl.textContent = `${available}/${capacity}`;
  }
  seatsRepairEl.textContent = '0';
}

function renderSeats(capacity, occupiedSeats) {
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  const leftRoomInput   = document.getElementById('roomNumber');  // hidden input
  ...
  // ── Room sync (right select → left hidden input) ─────────────────────────
  ...
  async function updateDisplay() {
    ...
    const capacity = getCapacityFromSelect(roomSelect);

    currentOccupiedSeats = await fetchOccupiedSeats(room, date, timeIn);
    updateBookingOverview(capacity, currentOccupiedSeats);
    renderSeats(capacity, currentOccupiedSeats);
  }
  ...
  async function performReservation() {
    ...
    if (convertTimeToMinutes(timeOut) <= convertTimeToMinutes(timeIn)) { alert('Time Out must be after Time In.'); return; }
    ...
    try {
      const res = await fetch('/admin/slots/reservation', {
        ...
        body: JSON.stringify({
          studentId, date, timeIn, timeOut, room,
          seats: selectedSeats,
      ...
      if (succeeded.length > 0) alert(`Successfully reserved seat(s): ${succeeded.map(r => r.seat).join(', ')}`);
      if (failed.length > 0)    alert(`Could not reserve seat(s): ${failed.map(r => `${r.seat} (${r.reason})`).join(', ')}`);
  ...
  async function performRemoval() {
    ...
    try {
      ...
      if (succeeded.length > 0) alert(`Successfully removed reservation(s) for seat(s): ${succeeded.map(r => r.seat).join(', ')}`);
      if (failed.length > 0)    alert(`Could not remove seat(s): ${failed.map(r => `${r.seat} (${r.reason})`).join(', ')}`);
